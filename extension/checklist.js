...
if (checklistRep.value) {
    ...
    var defaultNameList = defaultChecklist.map(function (item) { return item.name; });
    if (!lodash_1.isEqual(currentNameList, defaultNameList)) {
        if (checklistRep.value.every(function (item) { return item.complete; })) {
            checklistRep.value = defaultChecklist.map(function (item) { return ({
                name: item.name,
                complete: true,
            }); });
        }
...
nodecg.listenFor('toggleCheckbox', function (payload, ack) {
    toggleCheckbox(payload)
        .then(function (checklist) { return helpers_1.processAck(ack, null, checklist); })
